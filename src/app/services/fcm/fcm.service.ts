...
export class FcmService {
  ...
  addListeners() {
    PushNotifications.addListener(
      ...
      async(token: Token) => {
        ...
        let go = 1;
        const saved_token = JSON.parse((await this.storage.getFromStorage(FCM_TOKEN)).value);
        if(saved_token) {
        ...
        if(go == 1) {
          // save token
          this.storage.addToStorage(FCM_TOKEN, JSON.stringify(fcm_token));
        } else if(go == 2) {
          // update token
          const data = {
          ...
          this.storage.addToStorage(FCM_TOKEN, fcm_token);
        }
      }
    );
  ...
  async removeFcmToken() {
    try {
      const saved_token = JSON.parse((await this.storage.getFromStorage(FCM_TOKEN)).value);
      this.storage.removeFromStorage(saved_token);
    } catch(e) {
      console.log(e);
      throw(e);
    }
